        ...
        <div className="panel-dark relative overflow-hidden px-6 py-8 md:px-10 md:py-10">
          ...
          <div className="relative space-y-6">
            ...
            <p className="max-w-2xl text-base leading-8 text-slate-300 md:text-lg">
              Từ xe gấp đô thị gọn nhẹ, MTB chinh phục địa hình đến road
              tốc độ cao — tất cả đều mang tinh thần đại dương mạnh mẽ.
            </p>
                    ...
                    {categories.map((item) => {
                      ...
                      return (
                        <button
                          key={item.slug}
                          type="button"
                          onClick={() => setCategory(category === item.slug ? defaultValue : item.slug)}
                          className={cn(
                            "w-full rounded-full border px-4 py-2.5 text-left text-sm transition-colors",
                            category === item.slug
                              ? "border-primary bg-primary/10 font-medium text-primary"
            ...
            <Card className="panel-dark py-0">
              <CardContent className="space-y-4 p-5">
                ...
                <p className="text-sm leading-7 text-slate-300">
                  Liên hệ đại lý gần bạn để được tư vấn trực tiếp, test ride
                  và nhận hỗ trợ chọn size xe phù hợp.
                </p>
                <Link
                  href="/dai-ly"
                  className={cn(
          ...
          <div className="space-y-8">
            ...
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {filteredProducts.map((product) => (
                <Card key={product.slug} className="panel relative overflow-hidden py-0">
                  <CardContent className="space-y-4 p-5">
